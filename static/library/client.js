const params = new Proxy(new URLSearchParams(window.location.search), {
    get: (searchParams, prop)=>typeof prop === 'string' && searchParams.has(prop) ? searchParams.get(prop) : ""
});
...
if (redirect_to != null && redirect_to !== "") {
    document.querySelectorAll('input[type="hidden"][name="redirectTo"]').forEach((input)=>{
        if (input.value === "") {
...
function onEnhancedFormSubmit(event) {
    ...
    const formData = new FormData(form).entries().map(([key, value])=>[
            key,
            value + ''
        ]);
    const { email, password, passwordConfirm, redirectTo, action } = Object.fromEntries(formData);
}
document.querySelectorAll('form[data-is="form-enhanced"]').forEach((form)=>form.addEventListener("submit", onEnhancedFormSubmit));
const kebabize = (str)=>str.replace(/[A-Z]+(?![a-z])|[A-Z]/g, ($, ofs)=>(ofs ? "-" : "") + $.toLowerCase());
const createElementDom = (tagName, attributes, ...children)=>{
    const element = document.createElement(tagName);
    attributes && Object.entries(attributes).forEach(([k, v])=>{
        if (typeof v === "function") {
            ...
            if (Array.isArray(classValue)) {
                const classes = classValue.flat(Infinity).filter(Boolean);
    ...
    children && children.map((c)=>c != null && c !== false && element.append(Array.isArray(c) ? createElementDom(...c) : typeof c === "string" || typeof c === "number" ? document.createTextNode(c + "") : typeof c === "function" ? c(document.createTextNode(""), "textContent") ?? "" : c));
    return element;
};
const createElementTemplate = (createElement, tagName, baseAttributes)=>{
    return (attributes, ...children)=>{
        return createElement(tagName, baseAttributes != null ? {
            ...baseAttributes,
            ...attributes
        } : attributes, ...children);
    };
};
const createElementsProxy = (createElement)=>{
    const cache = new Map();
...
const isValidHTMLTag = (tagName)=>{
    return typeof document !== "undefined" ? document.createElement(tagName) instanceof HTMLElement : /^[a-z][a-z0-9-]*$/i.test(tagName);
};
const h = createElementsProxy(createElementDom);
...
const noOp = (v)=>v;
const Signal = (value, { transform = noOp } = {})=>{
...
const pathRegex = /^(?:(?<protocol>\w+):\/)?\/*(?<dir>.+\/)?(?<filename>(?<basename>.+?)(?:\.(?<extension>\w+))?)$/;
...
const splitPathStringToParts = (path)=>{
    path = path.replace(/^\/+/, "");
    const { protocol: _protocol, dir, filename, basename, extension } = path.match(pathRegex)?.groups ?? DEFAULT_REGEX_VALUES;
...
const MapSignal = (value, { transform = noOp } = {})=>{
    ...
    const getItem = (key)=>signal.get()[key];
    const hasItem = (key)=>key in signal.get();
    const removeItem = (key)=>{
        ...
        listenersForRemovals.forEach((fn)=>fn(key, oldData[key]));
        signal.set(newData);
    };
    const onKeyAdded = (fn)=>listenersForAdditions.add(fn);
    const offKeyAdded = (fn)=>listenersForAdditions.delete(fn);
    const onKeyRemoved = (fn)=>listenersForRemovals.add(fn);
    const offKeyRemoved = (fn)=>listenersForRemovals.delete(fn);
    const onKeyChanged = (fn)=>listenersForChange.add(fn);
    const offKeyChanged = (fn)=>listenersForChange.delete(fn);
    return {
        ...signal,
        setItem,
        getItem,
...
const LocalStorageSignalAdapter = (signal, localStorageKey)=>{
    const value = JSON.parse(localStorage.getItem(localStorageKey) || JSON.stringify(signal.get()));
    signal.set(value);
...
const userMessagesCollection = (()=>{
    const signal = LocalStorageSignalAdapter(MapSignal({}), "__userMessages");
    const add = (type, message, id)=>{
    ...
    const addSuccess = (message)=>add(TYPE_SUCCESS, message);
    const addError = (message)=>add(TYPE_ERROR, message);
    const addInfo = (message)=>add(TYPE_INFO, message);
    const addWarning = (message)=>add(TYPE_WARNING, message);
    const remove = (id)=>signal.removeItem(id);
    const get = (id)=>signal.getItem(id);
...
const toURLParams = (values)=>(values instanceof FormData ? Array.from(values.entries()) : Object.entries(values)).map(([k, v])=>`${k}=${encodeURIComponent(v + "")}`).join("&");
const disableForm = (form)=>{
    let length = form.elements.length;
    while(length--){
        const control = form.elements[length];
...
const handleSimpleFormSubmit = (callback)=>(event)=>{
        event.preventDefault();
...
const pickCorrectFetch = (type, serialized)=>{
    switch(type){
        case "json":
            {
...
const autoFetchWhenSubmit = (config = defaultConfig)=>{
    const { type, classBusy, doDisableForm, callback } = {
        ...config,
        ...defaultConfig
    };
    const useSerializedData = (serialized, form)=>{
...
const { button, img, li, span, div, ul, a, pre, code } = h;
const currentProps = {
    ariaCurrent: "true",
    ariaDisabled: "true"
};
...
const setupCarousel = (carouselWrapper)=>{
    ...
    const slides = Array.from(slider.querySelectorAll(".carouselSlidesListSlide")).map((slide)=>{
    ...
    const controller = (()=>{
        ...
        let currentSlide = Math.ceil(slider.scrollLeft / slider.offsetWidth) % slides.length;
        const nextSlide = ()=>updateSlider((currentSlide + 1) % slides.length);
        const prevSlide = ()=>updateSlider((currentSlide - 1 + slides.length) % slides.length);
    ...
    handlePause: {
        const isReduced = window.matchMedia(`(prefers-reduced-motion: reduce)`) != null && window.matchMedia(`(prefers-reduced-motion: reduce)`).matches === true;
        if (isReduced) {
            break handlePause;
        }
        const pauseToggleButton = carouselWrapper.querySelector(".carouselControlsButtonPlayToggle");
        const pauseToggleButtonTextElement = pauseToggleButton?.querySelector("span");
        ...
        const startTimer = ()=>{
            ...
            carouselWrapper.style.setProperty("--timer-time", `${8000}ms`);
            carouselWrapper.style.setProperty("--timer-animation-name", "carouselProgressAnimation");
        };
        const stopTimer = ()=>{
    ...
    const slideButtons = (()=>{
        const carouselSlidesNavigationContainer = carouselWrapper.querySelector(".carouselSlidesNavigation");
        ...
        const slideButtons = slides.map((slide, index)=>{
            ...
            slideButton.addEventListener("click", ()=>controller.updateSlider(index));
    ...
    setupNextPrevButtons: {
        carouselWrapper.querySelector(".buttonPrevious")?.addEventListener("click", controller.prevSlide);
        carouselWrapper.querySelector(".buttonNext")?.addEventListener("click", controller.nextSlide);
    }
};
...
document.querySelectorAll("button.clapsButton").forEach(function processClapsButton(button) {
    ...
    button.addEventListener("animationend", ()=>button.setAttribute("aria-pressed", "false"));
...
function onclipboardButtonClick(_event) {
    const value = this.getAttribute("data-clipboard");
    const clipboard = value === "currentURL" || value == null ? window.location.href : value;
...
document.querySelectorAll("button[data-is='clipboard-button']").forEach(processClipboardButton);
const classPrefix = "fileInText";
const splitHTMLElementTextContent = (holderElement)=>{
    ...
    add(recipient, span({
        class: `${classPrefix}Protocol`
    }, pathParts.protocol), span({
        class: `${classPrefix}Slash`
    }, "://"), pathParts.dirname.length > 0 ? pathParts.dirname.map((chunk)=>[
            span({
                class: `${classPrefix}Directory`
            }, chunk),
            span({
                class: `${classPrefix}Slash`
            }, "/")
        ]) : undefined, pathParts.filename && span({
        class: `${classPrefix}File`
    }, pathParts.filename));
};
document.querySelectorAll('[data-is="file-in-text"]').forEach(splitHTMLElementTextContent);
...
const makeRevealButton = (summary, details)=>{
    if (!details || !summary) {
        return;
    }
    ...
    const isOpen = summary.hasAttribute("data-open") && summary.getAttribute("data-open") === "true";
    const revealBtn = Object.assign(button({
...
document.querySelectorAll('[data-is-disclosure').forEach((el)=>{
    if (el.classList.contains(wasProcessedClass)) {
        return;
    }
    el.classList.add(wasProcessedClass);
    const summary = el.querySelector(".disclosureSummary");
    const details = el.querySelector(".disclosureDetails");
    if (!summary || !details) {
        return;
    }
...
const wasProcessedClass1 = "isJSProcessed";
const processCallout = (collapsiblesList)=>{
    if (collapsiblesList.classList.contains(wasProcessedClass1)) {
        return;
    }
    collapsiblesList.classList.add(wasProcessedClass1);
    const captions = collapsiblesList.querySelectorAll("dt");
    captions.forEach((caption)=>{
        const content = caption.nextElementSibling;
...
document.querySelectorAll('[data-is="collapsible-list"]').forEach(processCallout);
...
const resetFontSize = ()=>setBaseFontSize(15);
...
document.querySelectorAll(".mainSearch").forEach((searchWrapper)=>{
    const selectedTermsContainer = searchWrapper.querySelector(".mainSearchSelectedTerms");
    ...
    const addTerm = (id, name, labelText)=>{
        selectedTermsContainer.append(button({
    ...
    const options = Array.from(searchWrapper.querySelectorAll([
    ...
    ].join(","))).reduce((arr, label)=>{
        const input = document.getElementById(label.htmlFor);
        if (!input) {
            return arr;
        }
        input.addEventListener("change", onInputChange);
        if (input.value !== "" && urlParams.getAll(input.name).indexOf(input.value) !== -1) {
            input.checked = true;
    ...
    document.querySelector(".mainSearchFiltersSubjectTagsClearLink")?.addEventListener("click", ()=>{
        options.forEach((option)=>{
    ...
    document.querySelector(".mainSearchSelectedTermsClearAllButton")?.addEventListener("click", ()=>{
        options.forEach((option)=>{
            removeTerm(option.input.id);
        });
    });
});
function setPasswordButtonText(text) {
    this.buttonLabel.textContent = text;
}
function onclipboardButtonClick1() {
...
document.querySelectorAll('input[type="password"]').forEach((input)=>{
    ...
    const revealButon = Object.assign(button({
        class: 'passwordContainerRevealButton',
        ariaExpanded: 'false',
        type: 'button',
        ariaControls: id,
        onClick: onclipboardButtonClick1
    }, buttonLabel), {
        buttonLabel,
        setText: setPasswordButtonText
    });
    const container = div({
        class: [
            'passwordContainer',
...
const classPrefix1 = "userMessage";
const container = div({
    class: `${classPrefix1}sPane`
});
add(document.body, div({
    class: `${classPrefix1}s`
}, container));
const addUserMessageElement = (id, message, type)=>{
    const popup = div({
        role: "alert",
        ariaLive: "assertive",
        ariaAtomic: "true",
...
container.addEventListener("animationend", (event)=>{
    const target = event.target;
    if (target.classList.contains(`${classPrefix1}Closing`)) {
        target.remove();
    }
});
userMessagesCollection.entries().forEach(([id, { message, type }])=>addUserMessageElement(id, message, type));
userMessagesCollection.onMessageAdded((id, { message, type })=>addUserMessageElement(id, message, type));
userMessagesCollection.onMessageRemoved((id)=>document.getElementById(id)?.classList.add(`${classPrefix1}Closing`));
...
document.querySelectorAll(".nodeFlashCardShuffler").forEach((element)=>{
    const contentPane = element.querySelector(".nodeFlashCardShufflerContentPane");
    const buttons = element.querySelectorAll("button.nodeFlashCardShufflerButton");
...
document.querySelectorAll(".itemSummaryBrowser").forEach((element)=>{
    const randomButton = element.querySelector("button.itemSummaryBrowserRandomButton");
...
const processTogglerButton = (button)=>{
    if (!button || !(button.tagName === "BUTTON") || button.classList.contains("isJSProcessed")) {
        return;
    }
...
const TogglerButton = (props, ...children)=>{
...
document.querySelectorAll("button[data-is='toggler-button']").forEach(processTogglerButton);
const slugify = (text)=>text.toLowerCase().replace(/(\s|\.)+/g, "-");
const sortElementsByPosition = (a, b)=>(document.getElementById(a)?.getBoundingClientRect().top ?? 0) - (document.getElementById(b)?.getBoundingClientRect().top ?? 0);
...
const clickButtonIfFound = (element)=>{
    const button = element && Array.from(element.children).find((child)=>child.matches('button[aria-expanded="false"]'));
    if (button != null) {
        button.click();
    }
};
...
const processArticleToc = (root)=>{
    ...
    const headings = Array.from(document.querySelectorAll("main h1, main h2, .contributeBlockWrapper h1"));
    const headingIndicesById = headings.map((heading)=>{
        ...
        const listItem = li({
            ...
            href: `#${id}`,
            class: [
                ...Array.from(heading.classList).filter((c)=>c.toLowerCase().includes("icon")),
    ...
    const observer = new IntersectionObserver((entries)=>{
        ...
        let found = false;
        root.querySelectorAll(`.${CLASS_CURRENT_LIST_ITEM}`).forEach((el)=>el.classList.remove(CLASS_CURRENT_LIST_ITEM));
        for (const id of headingIndicesById){
            if (found || id === last) {
                ...
                root.querySelector(`[href="#${id}"]`)?.classList.remove(CLASS_ANCHOR_SCROLLED_PAST);
            }
        }
    }, {
        rootMargin: "0% 0% -10% 0%"
...
document.querySelectorAll("#tableOfContents button.tableOfContentsToggleButton").forEach(processNavigationBehavior);
function augmentTabList(tabList) {
    ...
    let tabFocus = -1;
    const cache = new Map();
    const tabs = Array.from(tabList.querySelectorAll('a[href^="#"][role="tab"]')).reduce((tabs, tab, index)=>{
...
document.querySelectorAll('button[is-theme-toggle]').forEach((button)=>{
    button.addEventListener('click', ()=>{
        const isDark = document.documentElement.classList.toggle('dark');
        localStorage.setItem('theme', isDark ? 'dark' : 'light');
    });
});
document.querySelectorAll(".showWhenJSLoads").forEach((element)=>element.removeAttribute("hidden"));
document.querySelectorAll(".makeUsableWhenJSLoads").forEach((element)=>element.style.removeProperty("pointer-events"));
document.querySelectorAll("a").forEach((a)=>a.getAttribute("href") == window.location.pathname && a.classList.add("active"));
